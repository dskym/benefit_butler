# Benefit Butler — Product Requirements Document
...
### 4. Card Performance & Recommendation
...
| 기능 | 상태 |
|------|------|
| 이메일 회원가입/로그인 (JWT) + 이메일 인증 | ✅ |
| 거래 CRUD + 카테고리 CRUD + 대시보드 + 분석 + 설정 | ✅ |
| AI 자동 카테고리 분류 (가맹점명 기반) | ✅ |
| 오프라인 퍼스트 (MMKV + 낙관적 업데이트 + 재시도 전략) | ✅ |
| Excel 가져오기/내보내기 (.xlsx, .xls) | ✅ |
| SMS/푸시 실시간 파싱 (Android) | ✅ |
| 카드 실적 트래커 + 카드 추천 알고리즘 | ✅ |
| 소셜 로그인 (카카오/구글) | ⬜ |
